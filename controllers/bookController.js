...
class BookController {
  async create(req, res, next) {
    try {
      let {name, pages, authorId, genre} = req.body;
      ...
      const book = await Book.create({name, picture: fileName, bookFile: bookFileName, pages, author: authorId, genre})
  ...
  async search(req, res) {
    ...
    let books = []
    if (query) books = await Book.find({name: {$regex: new RegExp(query, 'i')}}).populate('genre').populate('author')
    return res.json(books);
  }
